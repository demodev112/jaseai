...
exports.createKakaoToken = exports.resetDailyUsage = exports.analyzeVideo = void 0;
  ...
  "improvements": [
    {
      ...
      "correction": "구체적인 교정 방법"
    }
  ],
  "injuryRisk": "low, medium, 또는 high",
  "overallAdvice": "종합적인 조언 (1-2문장)"
}`;
// ─── analyzeVideo ───────────────────────────────────────
exports.analyzeVideo = (0, https_1.onCall)({
    ...
    if (!exerciseName || !videoStoragePath) {
        throw new https_1.HttpsError('invalid-argument', '운동 이름과 영상 경로가 필요합니다.');
    }
    // 3. Check daily limit
    ...
    if (dailyLimit === 0) {
        throw new https_1.HttpsError('permission-denied', '구독이 필요합니다.');
    }
    ...
    if (currentCount >= dailyLimit) {
        throw new https_1.HttpsError('resource-exhausted', `오늘의 분석 횟수를 모두 사용했어요. (${currentCount}/${dailyLimit}회)`);
    }
    ...
    try {
        ...
        if (!exists) {
            throw new https_1.HttpsError('not-found', '영상 파일을 찾을 수 없습니다.');
        }
        ...
        if (error instanceof https_1.HttpsError)
            throw error;
        throw new https_1.HttpsError('internal', error.message || 'AI 분석 중 오류가 발생했습니다.');
    }
});
// ─── resetDailyUsage ────────────────────────────────────
// Runs every day at midnight KST (15:00 UTC previous day)
exports.resetDailyUsage = (0, scheduler_1.onSchedule)({
...
exports.createKakaoToken = (0, https_1.onCall)({
    region: 'asia-northeast3',
}, async (request) => {
    const { kakaoUid, email, displayName } = request.data;
    if (!kakaoUid) {
        throw new https_1.HttpsError('invalid-argument', '카카오 UID가 필요합니다.');
    }
